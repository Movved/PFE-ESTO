...
<head>
    ...
    <style>
        ...
        input[type=text],input[type=email],input[type=password],select{padding:9px 12px;border:1px solid var(--border);border-radius:8px;font-size:13px;background:var(--background);color:var(--text-primary);outline:none;width:100%;box-sizing:border-box}
...
<div class="main">
    <header class="topbar"><span class="topbar-title">Ajouter un enseignant</span></header>
    <main class="content">
        <div class="page-header">
            <div><div class="page-title">Nouvel enseignant</div><div class="page-sub">Créer un nouveau compte enseignant</div></div>
        ...
        <div class="card">
            ...
            <form method="POST" action="{{ route('admin.enseignants.store') }}">
                ...
                <div class="card-body">
                    <div class="form-row">
                        <div class="form-group"><label>Prénom</label><input type="text" name="prenom" value="{{ old('prenom') }}" placeholder="Youssef">@error('prenom')<div class="form-error">{{ $message }}</div>@enderror</div>
                        <div class="form-group"><label>Nom</label><input type="text" name="nom" value="{{ old('nom') }}" placeholder="Benali">@error('nom')<div class="form-error">{{ $message }}</div>@enderror</div>
                    </div>
                    <div class="form-group"><label>Email</label><input type="email" name="email" value="{{ old('email') }}" placeholder="y.benali@ump.ac.ma">@error('email')<div class="form-error">{{ $message }}</div>@enderror</div>
                    <div class="form-group"><label>Spécialité</label><input type="text" name="specialite" value="{{ old('specialite') }}" placeholder="Informatique">@error('specialite')<div class="form-error">{{ $message }}</div>@enderror</div>
                    <div class="form-group">
                        <label>Département</label>
                        <select name="id_departement">
                            <option value="">-- Choisir un département --</option>
                            @foreach($departements as $d)
                                <option value="{{ $d->id_departement }}" {{ old('id_departement') == $d->id_departement ? 'selected' : '' }}>{{ $d->nom_departement }}</option>
                            @endforeach
                        </select>
                        @error('id_departement')<div class="form-error">{{ $message }}</div>@enderror
                    </div>
                    <div class="form-group"><label>Mot de passe</label><input type="password" name="mot_de_passe" placeholder="••••••••">@error('mot_de_passe')<div class="form-error">{{ $message }}</div>@enderror</div>
                    <label class="checkbox-row"><input type="checkbox" name="is_chef" {{ old('is_chef') ? 'checked' : '' }}> Chef de département</label>
                </div>
