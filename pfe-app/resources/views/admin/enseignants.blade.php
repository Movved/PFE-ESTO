...
<head>
    ...
    <style>
        ...
        .badge-chef{background:#EEF2FF;color:#3730A3}.badge-chef .badge-dot{background:#6366F1}
        .badge-active{background:#F0FBF4;color:#1A7A34}.badge-active .badge-dot{background:var(--success)}
        .badge-inactive{background:var(--background);color:var(--text-secondary)}.badge-inactive .badge-dot{background:var(--border)}
        ...
        .user-avatar-sm{width:30px;height:30px;border-radius:50%;background:#6366F1;color:white;font-size:11px;font-weight:600;display:flex;align-items:center;justify-content:center;flex-shrink:0}
...
<div class="main">
    ...
    <main class="content">
        ...
        <div class="page-header">
            <div><div class="page-title">Enseignants</div><div class="page-sub">{{ $enseignants->count() }} enseignant(s) enregistré(s)</div></div>
            <a href="{{ route('admin.enseignants.create') }}" class="btn btn-primary"><svg viewBox="0 0 24 24"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Ajouter un enseignant</a>
        </div>
        <div class="card">
            <div class="card-header"><div><div class="card-title">Liste des enseignants</div><div class="card-sub">Tous les comptes enseignants</div></div></div>
            <div style="overflow-x:auto;">
                <table>
                    <thead><tr><th>Enseignant</th><th>Spécialité</th><th>Département</th><th>Rôle</th><th>Statut</th><th>Actions</th></tr></thead>
                    <tbody>
                        @forelse($enseignants as $e)
                        <tr>
                            <td><div style="display:flex;align-items:center;gap:10px;"><div class="user-avatar-sm">{{ strtoupper(substr($e->prenom,0,1)) }}{{ strtoupper(substr($e->nom,0,1)) }}</div><div><div style="font-weight:500;">{{ $e->prenom }} {{ $e->nom }}</div><div style="font-size:11px;color:var(--text-secondary);">{{ $e->email }}</div></div></div></td>
                            <td style="font-size:12px;">{{ $e->specialite }}</td>
                            <td style="font-size:12px;color:var(--text-secondary);">{{ $e->nom_departement }}</td>
                            <td>@if($e->is_chef)<span class="badge badge-chef"><span class="badge-dot"></span>Chef</span>@else<span style="font-size:12px;color:var(--text-secondary);">Enseignant</span>@endif</td>
                            <td>@if($e->actif)<span class="badge badge-active"><span class="badge-dot"></span>Actif</span>@else<span class="badge badge-inactive"><span class="badge-dot"></span>Inactif</span>@endif</td>
                            <td><div style="display:flex;gap:6px;">
                                <a href="{{ route('admin.enseignants.edit', $e->id_enseignant) }}" class="btn btn-ghost"><svg viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>Modifier</a>
                                <form method="POST" action="{{ route('admin.enseignants.destroy', $e->id_enseignant) }}" onsubmit="return confirm('Supprimer cet enseignant ?')">@csrf @method('DELETE')<button type="submit" class="btn btn-danger"><svg viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/></svg>Supprimer</button></form>
                            </div></td>
                        </tr>
                        @empty<tr><td colspan="6"><div class="empty-state">Aucun enseignant trouvé.</div></td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
