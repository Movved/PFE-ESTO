    ...
    <div class="main">
        ...
        <main class="content">
            ...
            <div class="stats-grid">
                ...
                <div class="stat-card">
                    <div class="stat-icon orange"><svg viewBox="0 0 24 24">
                ...
                <div class="stat-card">
                    ...
                    <div>
                        <div class="stat-value">{{ isset($moyenneGlobale) ? number_format($moyenneGlobale, 1) : '—' }}
                        </div>
            ...
            <div class="section-grid">
                <div class="card">
                    <div class="card-header">
                        <div>
                            <div class="card-header-title">Mes Modules</div>
                            <div class="card-header-sub">Année académique en cours</div>
                        </div>
                        <a href="{{ route('enseignant.modules') }}" class="btn btn-secondary"
                            style="padding:5px 12px;font-size:13px;">Voir tout</a>
                    </div>
                    @forelse($modules ?? [] as $module)
                        <a href="{{ route('enseignant.module.show', $module->id_module) }}" class="module-item"
                            ...
                            <div class="module-item-info">
                                ...
                                <div class="module-item-sem"><span class="code-badge">{{ $module->code_module }}</span> ·
                                    Semestre {{ $module->semestre_numero ?? '—' }}</div>
                            </div>
                ...
                <div class="card">
                    <div class="card-header">
                        <div>
                            ...
                            <div class="card-header-sub">Tous modules confondus</div>
                        </div>
                    </div>
                    <div class="card-body">
                        @php
                        ...
                        <div class="rep-list">
                            ...
                            <div class="rep-total"><span>Total notes saisies</span><strong>{{ $rep['total'] }}</strong>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <a href="{{ route('enseignant.reclamations') }}" class="btn btn-secondary"
                        style="padding:5px 12px;font-size:13px;">Voir tout</a>
                </div>
                    ...
                    <table>
                        ...
                        <tbody>
                                ...
                                <tr>
                                        ...
                                        <div class="etu-cell">
                                            ...
                                            <div>
                                                <div style="font-size:14px;font-weight:500;">
                                                    {{ $rec->prenom_etudiant ?? '—' }} {{ $rec->nom_etudiant ?? '' }}</div>
                                                <div style="font-size:12px;color:var(--text-secondary);">
                                                    {{ $rec->cne_etudiant ?? '' }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td><span class="module-name">{{ $rec->nom_module ?? '—' }}</span><br><span
                                            class="code-badge">{{ $rec->code_module ?? '' }}</span></td>
                                    <td class="center">
                                        @if(isset($rec->note))
                                            <span
                                                class="grade-value {{ $rec->note >= 12 ? 'grade-pass' : ($rec->note >= 10 ? 'grade-warn' : 'grade-fail') }}">{{ number_format($rec->note, 2) }}</span>
                                        @else
                                            <span class="grade-empty">—</span>
                                        @endif
                                    </td>
                                    <td>
                                        <div class="rec-msg" title="{{ $rec->message ?? '' }}">{{ $rec->message ?? '—' }}
                                        </div>
                                    </td>
                                    <td class="center"><span
                                            style="font-size:13px;color:var(--text-secondary);">{{ isset($rec->date_reclamation) ? \Carbon\Carbon::parse($rec->date_reclamation)->format('d/m/Y') : '—' }}</span>
                                    </td>
                                    <td class="center">
                                    ...
                                    <td class="center">
                                        <button class="btn-voir"
                                            onclick="openModal({{ $rec->id_reclamation }}, '{{ addslashes(($rec->prenom_etudiant ?? '') . ' ' . ($rec->nom_etudiant ?? '')) }}', '{{ addslashes($rec->nom_module ?? '') }}', {{ $rec->note ?? 'null' }}, `{{ addslashes($rec->message ?? '') }}`)">
                                            Voir
                                        </button>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="7">
                                        <div class="empty-state"><svg viewBox="0 0 24 24">
                                                <path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9" />
                                                <path d="M13.73 21a2 2 0 01-3.46 0" />
                                            </svg>Aucune réclamation pour le moment.</div>
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
    ...
    <div class="modal-overlay" id="rec-modal" onclick="if(event.target===this)closeModal()">
        <div class="modal">
            ...
            <div class="modal-body">
                ...
                <div style="font-size:13px;color:var(--text-secondary);margin-bottom:6px;font-weight:500;">Message de
                    l'étudiant</div>
                <div id="modal-msg" class="modal-msg-box"></div>
                <form method="POST" action="{{ route('enseignant.reclamations.traiter', ['id' => 0]) }}" id="rec-form"
                    style="margin-top:16px;">
                    @csrf
                    @method('PATCH')
                    <input type="hidden" name="id_reclamation" id="modal-rec-id">
                    <div style="display:flex;flex-direction:column;gap:6px;">
                        <label style="font-size:13px;font-weight:500;color:var(--text-primary);">Réponse /
                            Commentaire</label>
                        <textarea name="reponse" rows="3" placeholder="Expliquez votre décision..."
